...
final class Router
{
    ...
    public function dispatch(): void
    {
        ...
        // Pages services direct (nouveau format SEO)
        if (preg_match('#^/services/([a-z0-9\-]+)$#', $uri, $m)) {
            $this->servicePage($m[1]);
            return;
        }
    ...
    private function home(): void
    {
        ...
        // Vérifier si le nouveau template existe
        $newTemplate = 'home-green';
        $templateFile = __DIR__ . '/../templates/' . $newTemplate . '.php';
        
        if (is_file($templateFile)) {
            $this->view()->render($newTemplate, compact('seo'));
        } else {
    ...
    private function heatingPage(string $slug): void
    {
        ...
        // Vérifier si un template dédié existe dans services/
        $serviceTemplate = 'services/' . $slug;
        $templateFile = __DIR__ . '/../templates/' . $serviceTemplate . '.php';
        ...
        // Si un template spécifique existe, l'utiliser
        if (is_file($templateFile)) {
            $this->view()->render($serviceTemplate, compact('service', 'seo'));
        } else {
    ...
    private function servicePage(string $slug): void
    {
        // Rediriger vers la page chauffage correspondante
        $this->redirect('/chauffage/' . $slug);
    }
